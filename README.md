...
- **局部软注意力（AGAT 层）**：
  ...
  1. **分子部分：** $\exp(a_{i,j})$   是对节点 $ i $ 和节点 $j$ 之间的原始注意力分数 $ a_{i,j} $ 进行指数化处理。指数化处理的目的是将原始分数转换为正数，并放大差异，使得重要的节点具有更大的权重。 
  2. **分母部分：** $\sum_{k \in \mathcal{N}_i} \exp(a_{i,k})$   是对节点 $i$ 的所有邻域节点 $ k $ 的指数化注意力分数进行求和。求和的目的是对注意力权重进行归一化，使得所有邻域节点的注意力权重之和为 1。 
